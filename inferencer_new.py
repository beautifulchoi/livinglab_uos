...
class Inferencer(object):
    ...
    def __call__(self, k: int = 1, infer_per_num=5, batch_size=32):
        ...
        if self.mode == 'single':
            with torch.no_grad():
                ...
                pred = prob(pred)
                # 한 배치의 topk정보 지님, 형태는 [value= batch size*5 형태, indices=b size*5 형태]
                pred_topk = torch.topk(pred, k)
                batch_scores = pred_topk[0]
                batch_indices = pred_topk[1]
            # {"top k": {"Species":"라벨 이름", "percent":~, "plantNo":"카테고리 인덱스", "plantImgs":[샘플파일이름]}}
            result_dict = {}
            ...
            for n in range(k):
                ...
                # 라벨
                result_dict[f'top{n+1}']["Species"] = idx2class[batch_indices[0][n].item()]

                # 스코어의 소맥 결괏값
                result_dict[f'top{n+1}']["Percent"] = batch_scores[0][n].item()
                # 라벨의 인덱스
                result_dict[f'top{n+1}']["PlantNo"] = self.cat_info[result_dict[f'top{n+1}']["Species"]]

                infer_path = "/home/files/datasets/livinglab_cp/val/" + \
                    idx2class[batch_indices[0][n].item()]  # 추론 카테고리 파일 경로
                imgs, sample_paths = self.bring_imgs(
                    infer_path, img_num=infer_per_num, size=224)  # 샘플링
                infer_imgs += imgs  # 추론한 사진
                # 추론한 사진 경로
                result_dict[f'top{n+1}']["plantImgs"] = sample_paths
    ...
    def bring_imgs(self, path: str, img_num: int, size: int = 224):
        ...
        if img_num > len(img_list):
            print('이미지의 갯수가 입력값보다 적어서 폴더 내 모든 사진을 받아옵니다. 사진 갯수: {0}'.format(
                len(img_list)))
            img_num = len(img_list)
        # 해당 카테고리 사진들중 랜덤하게 img_num개 뽑아옴
        sampling_list = np.array(random.sample(img_list, img_num))
        images = [0]*img_num
        img_path = []
        ...
        # (224,224,3) 인 이미지들들을 리스트로 묶어서 반환시킴 / image path 리스트 반환 추가
    ...
    def make_table(self, images: list, shape: tuple):  # images 는 이미지들 담겨있는 리스트임
        ...
        try:
            ...
            for row_idx in range(shape[0]):  # row_idx 는 0,1,2
                # 이러면 images[0] [5] [10] 가져옴
                image_table = images[row_idx*shape[1]]
